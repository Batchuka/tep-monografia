...
Plantas de processo contínuo como o TEP não podem ser operadas apenas por malhas de controle locais. A literatura consolidada de \textbf{controle plant-wide} \cite{Larsson2000, Skogestad2004} estabelece que é necessária uma camada supervisória com visão holística da planta, capaz de coordenar as malhas locais e reagir a distúrbios que transcendem os limites de cada controlador individual. Skogestad formaliza essa hierarquia em três camadas: controle regulatório (malhas PID locais), controle supervisório (coordenação e mudanças de setpoints), e otimização em tempo real (reconfiguração de objetivos operacionais). Essa estrutura hierárquica é exatamente aquela que o laboratório proposto neste trabalho pretende implementar e validar.

Este projeto é desenvolvido sob a ênfase de \textbf{Integração de Sistemas de Controle}, refletindo uma abordagem centrada em interoperabilidade, padrões abertos e comunicação entre componentes heterogêneos. Embora o trabalho aborde problemas de automação industrial e tenha relevância para as disciplinas de Controle e Instrumentação, seu diferencial reside na perspectiva de arquitetura de sistemas: como integrar simuladores, interfaces de usuário, orquestradores de infraestrutura e controladores em uma solução coesa, reprodutível e extensível. A literatura recente mostra que essa perspectiva é válida e necessária: levantamentos sobre computação cloud-native \cite{Shuiguang2023} demonstram que os mesmos princípios de orquestração que tornaram o ecossistema de serviços de nuvem escalável e resiliente podem ser aplicados a sistemas de controle industrial, especialmente quando a separação entre estado desejado e estado executado é formalizada.

Trabalhos como \citeonline{Johansson2022} e \citeonline{Mellado2022} estabelecem precedentes concretos: o primeiro demonstra a orquestração de sistemas de controle distribuídos de alta disponibilidade via Kubernetes; o segundo propõe um CLP conteinerizado — o IoT-PLC — que encapsula cada funcionalidade em um contêiner separado e usa um modelo de dispositivo virtual para abstrair a interface com camadas superiores. Paralelamente, o padrão de \textit{Operators} do Kubernetes \cite{Burns2016} permite codificar lógica supervisória como controladores declarativos que observam o estado de uma aplicação e agem para manter a operação dentro de parâmetros definidos. A separação entre política (declarada em um recurso customizado) e execução (implementada no \textit{controller loop}) é análoga à separação entre especificação e runtime em arquiteturas de controle distribuído segundo a norma IEC~61499 \cite{IEC61499}, e a infraestrutura de alta disponibilidade do Kubernetes garante que o supervisor continue operando mesmo em face de falhas de componentes individuais.
...
\begin{enumerate}
  \item \textbf{Integração com Kubernetes:} um Operator básico em Go com Kubebuilder codifica lógica supervisória declarativa segundo os princípios do IEC~61499 \cite{IEC61499}, demonstrando viabilidade arquitetural sem implementação completa de estratégias de controle;
  \item \textbf{Controle \textit{plant-wide}:} proposta de hierarquia de controle (regulatório, supervisório, otimização) estruturada segundo Skogestad \cite{Skogestad2004}, com infraestrutura preparada mas não testada;
  \item \textbf{Integração OPC UA:} mapeamento do espaço de endereçamento e exposição das variáveis de processo via protocolo IEC~62541 \cite{IEC62541_1}, viabilizando integração futura com sistemas industriais reais;
  \item \textbf{Índice de Preditividade:} framework para cálculo da métrica desenvolvida no CERN \cite{Bradu2018}, preparado para análise de qualidade de malhas em trabalhos subsequentes.
\end{enumerate}
...
Além desse núcleo validado, o trabalho estabelece a infraestrutura para futuras extensões por meio de código básico (não validado) para: (i) integração com Kubernetes via Operator declarativo alinhado ao IEC~61499; (ii) propostas de controle \textit{plant-wide} segundo a hierarquia de Skogestad \cite{Skogestad2004}; (iii) integração OPC UA para compatibilidade com sistemas industriais reais; e (iv) framework de cálculo do Índice de Preditividade para avaliação de qualidade de malhas \cite{Bradu2018}. Esta estratégia bifurcada permite entregar um laboratório funcional e imediatamente útil, enquanto pavimenta direções claras para pesquisa subsequente.
