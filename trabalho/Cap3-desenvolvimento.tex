...
O repositório \texttt{tep-operator} implementa um Operator Kubernetes em Go, construído com o framework Kubebuilder. Seu objetivo é atuar como supervisor de alto nível da planta TEP, avaliando o estado das variáveis de processo e aplicando ações corretivas de acordo com a política declarada no CRD \texttt{PLCMachine}.
...
\begin{enumerate}
  ...
  \item \textbf{Ação corretiva}: se alguma variável estiver fora da faixa, calcula a ação de controle e chama \texttt{UpdateController}.
  \item \textbf{Atualização de status}: grava o estado observado em \texttt{PLCMachine.status}.
\end{enumerate}
