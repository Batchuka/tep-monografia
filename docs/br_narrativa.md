...
É nesse ponto que a analogia com Kubernetes se torna tecnicamente relevante. Eu parti para encontrar um artigo que me ajudasse  construir isso, encontrei 'art12_Cloud-Native-Computing-A-Survey-from-the-Perspective-of-Services_Shuiguang_et_al', que traz a definição de Cloud-Native e também outras definições. Kubernetes pode ser entendido como uma plataforma de supervisão declarativa para sistemas computacionais distribuídos. Nele, não se programa apenas uma sequência fixa de comandos; declara-se um estado operacional desejado, e a plataforma passa a observar continuamente o estado real do sistema para reduzi-lo à coerência com essa declaração.
    ...
    Na prática, essa supervisão tem consequências físicas sobre a infraestrutura computacional: processos são alocados em máquinas específicas, instâncias são criadas ou removidas conforme a política declarada, processos falhos são reiniciados, cargas são redistribuídas quando nós ficam indisponíveis, configurações são aplicadas ao ambiente de execução, limites de CPU e memória são impostos, rotas de comunicação são organizadas e mecanismos de tolerância a falhas são acionados para manter o serviço disponível.

    Portanto, por trás das abstrações de alto nível do Kubernetes, há uma atuação concreta sobre servidores, processos, memória, rede e armazenamento. O Kubernetes pode oferecer um paradigma de supervisão declarativa: observar continuamente um sistema distribuído, comparar seu estado atual com um estado desejado e executar ações corretivas para preservar a operação. Essa interpretação é sustentada pela literatura sobre Borg, Omega e Kubernetes, especialmente na forma como esses sistemas tratam gerenciamento de aplicações distribuídas, estado desejado, reconciliação e automação operacional.
