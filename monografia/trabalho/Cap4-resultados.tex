...
\begin{lstlisting}[float=htbp, label={lst:operator_log}, caption={Log do \texttt{tep-operator} após conectividade estabelecida (Experimento 12)}]
...
\begin{figure}[htbp]
\centering
\missingfigure[figwidth=0.92\textwidth]{Gráfico do Experimento 15 (IDV(1)): eixo X = tempo simulado (0 a 2{,}5~h); eixo Y esquerdo = XMEAS$_7$ pressão do reator (kPa), subindo monotonicamente de $\approx$2700~kPa até o limite ISD em $\approx$2960~kPa; eixo Y direito = XMV$_6$ purge valve (\%), subindo de $\approx$39\% para $\approx$62\% sem conseguir conter o acúmulo. Marcar com linha vertical tracejada o instante de ISD em $t \approx 2{,}5$~h. Destaque visual: XMEAS$_9$ (temperatura) mantida absolutamente flat como evidência da armadilha diagnóstica.}
\caption{Experimento 15 --- IDV(1): pressão do reator (XMEAS$_7$) e abertura da purge (XMV$_6$) com marcação do evento ISD em $t \approx 2{,}5$~h.}
\label{fig:exp15_pressure}
\end{figure}
...
\begin{figure}[htbp]
\centering
\missingfigure[figwidth=0.92\textwidth]{Gráfico do Experimento 16 (IDV(2)): eixo X = tempo simulado (0 a $\approx$31~h); eixo Y esquerdo = XMEAS$_7$ pressão do reator (kPa); eixo Y direito = XMV$_6$ purge valve (\%). Evidenciar dois regimes com anotação ou fundo diferenciado: \textbf{Regime 1 — transiente rápido} ($t < 10$~h): pressão sobe $\approx$110~kPa em degrau e aparenta estabilizar em $\approx$2840~kPa, XMV$_6$ abre de 42 para 53\%; \textbf{Regime 2 — deriva lenta} ($t > 10$~h): pressão continua subindo monotonicamente a $\approx$4--5~kPa/h, XMV$_6$ segue abrindo até $>$67\%; marcação do ISD em $t \approx 31$~h. Comparar visualmente com a curva do Experimento 15 para evidenciar diferença de escala temporal.}
\caption{Experimento 16 --- IDV(2): dois regimes de resposta --- transiente rápido aparente e deriva lenta com colapso em $t \approx 31$~h.}
\label{fig:exp16_pressure}
\end{figure}
