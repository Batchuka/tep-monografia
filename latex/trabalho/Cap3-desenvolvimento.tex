...
\begin{figure}[htbp]
\centering
\missingfigure[figwidth=0.92\textwidth]{Diagrama de arquitetura do laboratório TEP-CPS: quatro componentes (\texttt{tep-plant}, \texttt{tep-operator}, \texttt{tep-ihm}, \texttt{tep-supervisor}) distribuídos em três camadas (campo, supervisão, monitoramento). Conexões: \texttt{tep-plant} → \texttt{tep-ihm} via gRPC \texttt{StreamMetrics} (streaming); \texttt{tep-operator} → \texttt{tep-plant} via gRPC \texttt{UpdateController} (bidirecional de controle); \texttt{tep-ihm} → navegador via WebSocket. Indica o fluxo de dados unidirecional e destaca que o operator não conhece a IHM e a planta não conhece nenhum dos dois.}
\caption{Arquitetura do laboratório TEP-CPS: componentes, camadas e protocolos de comunicação.}
